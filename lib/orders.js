...
export async function getAgentOrders(sessionTags = [], sessionEmail = '') {
  ...
  // 2. Pull all B2B orders and keep only those whose customer email is one of the agent's.
  const all = await getAllB2BOrders();
  const mine = (arr) => (arr || []).filter(o => myEmails.has((o.customer?.email || '').toLowerCase()));

  return {
    ...all,
    completedOrders: mine(all.completedOrders),
    draftOrders: mine(all.draftOrders),
  };
}
