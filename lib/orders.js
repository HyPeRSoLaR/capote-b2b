...
export async function getAgentClientEmails(sessionTags = [], sessionEmail = '') {
  const myAgentTags = (sessionTags || [])
    .map(t => t.toLowerCase())
    .filter(t => t.startsWith('agent_'));

  // Resolve which customer EMAILS belong to this agent (match any of their agent_* tags).
...
export async function getAgentOrders(sessionTags = [], sessionEmail = '') {
  ...
  const myEmails = await getAgentClientEmails(sessionTags, sessionEmail);

  // Add the agent's own email so agents see their own orders too
  if (sessionEmail) {
    myEmails.add(sessionEmail.toLowerCase().trim());
  }
  ...
  // Also resolve agent's own customer ID if available to catch orders with customer account linkage
  let sessionCustId = null;
  if (sessionEmail) {
    try {
      const custData = await shopifyGraphQL(
        `query GetAgentCustId($query: String!) {
      ...
      sessionCustId = custData.customers?.edges?.[0]?.node?.id?.split('/').pop();
    } catch {}
  }

  // 2. Query Shopify scoped to the agent's client emails (orders & draftOrders).
  const emailParts = Array.from(myEmails).map(e => `email:'${e}'`);
  if (sessionCustId) {
    emailParts.push(`customer_id:${sessionCustId}`);
  }
  const ordersEmailQuery = emailParts.join(' OR ');
